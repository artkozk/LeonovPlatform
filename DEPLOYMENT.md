...
## 18. Auth/Register pending timeout hardening (append-only, 2026-05-06)

### 18.1 Наблюдаемая проблема

1. Пользовательский UI на `/auth` мог оставаться в состоянии `Подождите...` без явного сообщения об ошибке, если API порт недоступен по сети или если запрос авторизации зависает по сети.
2. На практике это проявляется как "зависшая" регистрация/вход при отсутствии ответа от backend в разумный срок.

### 18.2 Что изменено

1. В frontend HTTP-клиент (`frontend/src/api/client.ts`) добавлен глобальный таймаут `REQUEST_TIMEOUT_MS=15000` для всех API-запросов.
2. Для `POST /auth/refresh` добавлен такой же явный таймаут, чтобы авто-refresh не зависал бесконечно.
3. В auth-store (`frontend/src/store/auth.ts`) добавлена нормализация сетевых ошибок:
- `ECONNABORTED` -> человекочитаемое сообщение о timeout;
- network/fetch errors -> сообщение о недоступности сервера авторизации;
- backend-ошибки по-прежнему читаются из `response.data.error`.

### 18.3 Почему сделано именно так

1. Пользователь не должен видеть вечный loading-state без диагноза: таймаут переводит зависший network-call в контролируемую ошибку.
2. Явный текст ошибки ускоряет triage: сразу понятно, что проблема в доступности API/маршрутизации, а не в полях формы.
3. Такой подход снижает количество ложных повторных кликов/дублирующих запросов и уменьшает нагрузку на поддержку.

### 18.4 Проверка после выката
...
## 20. Protected domain lock (`leonovcare.ru`) (append-only, 2026-05-13)

### 20.1 Новое обязательное правило

1. `leonovcare.ru` — защищённый домен другого сайта.
2. LeonovCarePlatform не должен публиковаться на этом домене.
3. Прод/стейдж контур платформы работает на отдельном адресе (`85.198.82.221:8511` или отдельный dev/stage-домен).

### 20.2 Что изменено в deploy-script

1. В `deploy/server/deploy.sh` добавлен preflight `enforce_domain_lock`.
2. Проверка выполняется до сборки/миграций.
3. Деплой завершается с `exit 1`, если nginx-конфиг `leonovcare.ru`:
- ссылается на `root /opt/leonovcare-platform/current/frontend/dist`;
- содержит прокси на `127.0.0.1:8510`;
- содержит прокси на `127.0.0.1:8511`.
4. Проверка работает в fail-closed режиме:
- если файл `/etc/nginx/sites-available/leonovcare.ru` отсутствует, деплой останавливается.

### 20.3 Управление проверкой

1. По умолчанию `DOMAIN_LOCK_ENABLED=true`.
2. Принудительное отключение возможно только вручную:
```bash
export DOMAIN_LOCK_ENABLED=false
```
3. Отключение допустимо только как осознанная операция с фиксацией причины в release-note, иначе это нарушает доменную политику.

### 20.4 Почему это сделано

1. Инцидент с подменой основного домена относится к критичным production-рискам.
2. Технический блок в deploy-flow исключает повтор ситуации даже при ручной ошибке в nginx или в автоматизации.
3. Fail-closed режим выбран, чтобы неизвестное состояние домена не проходило в релиз как «допустимое».
