...
def base_step(lesson_id: str, order: int, title: str, typ: str, diff: int, xp: int, minutes: int, body: str, topics: list[str], **extra: Any) -> dict[str, Any]:
    item = {
        ...
        "admin_notes": f"v17 pedagogical-ready regenerated from roadmap skeleton after independent QA; topics={', '.join(topics)}",
...
def post_process_course(course: dict[str, Any]) -> None:
    course["package_version"] = "v17_PEDAGOGICAL_READY"
    course["course"]["id"] = "python_backend_ai_v17_pedagogical_ready"
    ...
    course["course"]["source_note"] = "v17 uses the approved roadmap order and adds lesson_stage metadata, richer theory, AI-specific tasks, deeper debugging and project artifacts before import."
    for _, lesson in iter_lessons(course):
...
def build_course() -> dict[str, Any]:
    ...
    course = {
        "schema_version": "1.0",
        "package_version": "v17_PEDAGOGICAL_READY",
        "course": {
            "id": "python_backend_ai_v17_pedagogical_ready",
            "title": TITLE,
            ...
            "target_level": TARGET,
            "source_note": "Lesson order follows the approved roadmap skeleton. v17 adds lesson_stage metadata, AI-specific tasks, richer theory and stronger learning progression.",
            "modules": modules,
        },
    }
...
def early_future_knowledge_violation(st: dict[str, Any], lesson: dict[str, Any]) -> bool:
    if not (lesson["id"].startswith("v16_m01") or lesson["id"].startswith("v17_m01")) or lesson["order"] > 8 or st["type"] != "practice":
        return False
...
def write_docs(course: dict[str, Any]) -> None:
    preview = [f"# {TITLE}: v17_PEDAGOGICAL_READY", "", "Этот preview нужен для методиста. Импортировать нужно только `course_import.json`.", "", "v16 пересобран под учебную лестницу: зачем, объяснение с нуля, 3+ вопроса, практика, debugging, integration/project и итог проверяются валидатором.", ""]
    cmap = ["# Карта курса v17_PEDAGOGICAL_READY", ""]
    for module, lesson in iter_lessons(course):
        if lesson["order"] == 1:
...
def write_reports_fn(result: dict[str, Any]) -> None:
    s = result["stats"]
    lines = [
        "# Validation report v17_PEDAGOGICAL_READY", "", f"final status: {result['status']}", "",
    ...
    qa = [
        "# QA report v17_PEDAGOGICAL_READY", "", f"PASS/FAIL: {result['status']}", "",
