...
## 14. Update 2026-05-05 — loading hang fix for "Загрузка курсов и задач..."

1. Симптом:
- у части пользователей plugin мог очень долго оставаться на экране `Загрузка курсов и задач...`, особенно при большом количестве курсов/уроков.

2. Техническая причина:
- `TaskManager.refreshAll()` до фикса загружал задачи последовательно для всех курсов до первого рендера ready-state;
- если хотя бы один курс отвечал медленно или fallback уходил в lesson-fanout, UI оставался в loading слишком долго.

3. Что изменено:
- сначала загружаются `courses` и задачи только выбранного курса;
- ready-state публикуется сразу после этого шага;
- оставшиеся курсы догружаются в фоне и обновляют cache без блокировки initial UI;
- добавлен явный per-request timeout (`12s`) на загрузку курсов/задач в `TaskManager`.

4. Почему сделано именно так:
- это убирает критический UX-блокер при старте плагина;
- даже при медленных вторичных курсах пользователь начинает работать с выбранным курсом без долгого ожидания;
- поведение становится предсказуемым: либо быстрый ready-state, либо явная ошибка/офлайн fallback, а не бесконечная загрузка.
