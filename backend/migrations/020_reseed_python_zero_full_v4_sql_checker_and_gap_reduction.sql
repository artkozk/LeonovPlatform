-- 020_reseed_python_zero_full_v4_sql_checker_and_gap_reduction.sql
...
-- Required for SQL tasks reseeded by this migration.
...
$m2_l11_t4_solution$, 2, 50, $m2_l11_t4_topic$Месяц 2. Python Hard — Context$m2_l11_t4_topic$, $m2_l11_t4_lang$python$m2_l11_t4_lang$, $m2_l11_t4_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l11_t4_policy$::jsonb),
  ($m2_l11_t6_title$Resource$m2_l11_t6_title$, $m2_l11_t6_statement$**Коротко:** безопасный ресурс.

### Условие

Создай контекстный менеджер `Resource`. При входе печатает `open`, при выходе `close`, внутри блока `use`.

---$m2_l11_t6_statement$, $m2_l11_t6_starter$$m2_l11_t6_starter$, $m2_l11_t6_solution$class Resource:
    def __enter__(self):
        print("open")
        return self
...
with Resource():
    print("use")
$m2_l11_t6_solution$, 2, 50, $m2_l11_t6_topic$Месяц 2. Python Hard — Context$m2_l11_t6_topic$, $m2_l11_t6_lang$python$m2_l11_t6_lang$, $m2_l11_t6_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l11_t6_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  WHERE t.title IN (
    ...
    $m2_l11_ttitle_6$Resource$m2_l11_ttitle_6$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
work
  ...
  ($m2_l11_test_6_1_title$Resource$m2_l11_test_6_1_title$, $m2_l11_test_6_1_input$$m2_l11_test_6_1_input$, $m2_l11_test_6_1_expected$open
use
close
$m2_l11_test_6_1_expected$, FALSE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
---$m2_l11_b6_content$, $m2_l11_b6_task_title$Resource$m2_l11_b6_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m2_l12_t2_title$Start$m2_l12_t2_title$, $m2_l12_t2_statement$**Коротко:** запусти функцию в потоке.

### Условие

Создай функцию `worker`, которая печатает `work`. Запусти её через `threading.Thread`.$m2_l12_t2_statement$, $m2_l12_t2_starter$$m2_l12_t2_starter$, $m2_l12_t2_solution$import threading
...
thread.start()
thread.join()
$m2_l12_t2_solution$, 2, 50, $m2_l12_t2_topic$Месяц 2. Python Hard — Threads$m2_l12_t2_topic$, $m2_l12_t2_lang$python$m2_l12_t2_lang$, $m2_l12_t2_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l12_t2_policy$::jsonb),
  ($m2_l12_t4_title$I/O пример$m2_l12_t4_title$, $m2_l12_t4_statement$**Коротко:** запусти несколько потоков.

### Условие

Создай три потока. Каждый должен напечатать `done`. Итоговый вывод должен содержать три строки `done`.$m2_l12_t4_statement$, $m2_l12_t4_starter$$m2_l12_t4_starter$, $m2_l12_t4_solution$import threading
...
for thread in threads:
    thread.join()
$m2_l12_t4_solution$, 2, 50, $m2_l12_t4_topic$Месяц 2. Python Hard — Threads$m2_l12_t4_topic$, $m2_l12_t4_lang$python$m2_l12_t4_lang$, $m2_l12_t4_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l12_t4_policy$::jsonb),
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m2_l12_ttitle_2$Start$m2_l12_ttitle_2$,
    $m2_l12_ttitle_4$I/O пример$m2_l12_ttitle_4$,
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m2_l12_test_2_1_title$Start$m2_l12_test_2_1_title$, $m2_l12_test_2_1_input$$m2_l12_test_2_1_input$, $m2_l12_test_2_1_expected$work
$m2_l12_test_2_1_expected$, FALSE, 1),
  ($m2_l12_test_4_1_title$I/O пример$m2_l12_test_4_1_title$, $m2_l12_test_4_1_input$$m2_l12_test_4_1_input$, $m2_l12_test_4_1_expected$done
done
done
$m2_l12_test_4_1_expected$, FALSE, 1),
...
Создай функцию `worker`, которая печатает `work`. Запусти её через `threading.Thread`.$m2_l12_b2_content$, $m2_l12_b2_task_title$Start$m2_l12_b2_task_title$, NULL::jsonb),
...
Создай три потока. Каждый должен напечатать `done`. Итоговый вывод должен содержать три строки `done`.$m2_l12_b4_content$, $m2_l12_b4_task_title$I/O пример$m2_l12_b4_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m2_l14_t3_title$sleep$m2_l14_t3_title$, $m2_l14_t3_statement$**Коротко:** запусти async-функцию.

### Условие

Создай async-функцию `main`, которая печатает `start`, делает `await asyncio.sleep(0)`, затем печатает `end`.$m2_l14_t3_statement$, $m2_l14_t3_starter$$m2_l14_t3_starter$, $m2_l14_t3_solution$import asyncio
...
asyncio.run(main())
$m2_l14_t3_solution$, 2, 50, $m2_l14_t3_topic$Месяц 2. Python Hard — Asyncio$m2_l14_t3_topic$, $m2_l14_t3_lang$python$m2_l14_t3_lang$, $m2_l14_t3_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l14_t3_policy$::jsonb),
  ($m2_l14_t4_title$gather$m2_l14_t4_title$, $m2_l14_t4_statement$**Коротко:** запусти задачи вместе.

### Условие

Создай async-функцию `worker(name)`, которая печатает имя. Запусти три задачи через `asyncio.gather`: `A`, `B`, `C`.$m2_l14_t4_statement$, $m2_l14_t4_starter$$m2_l14_t4_starter$, $m2_l14_t4_solution$import asyncio
...
asyncio.run(main())
$m2_l14_t4_solution$, 2, 50, $m2_l14_t4_topic$Месяц 2. Python Hard — Asyncio$m2_l14_t4_topic$, $m2_l14_t4_lang$python$m2_l14_t4_lang$, $m2_l14_t4_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l14_t4_policy$::jsonb),
  ($m2_l14_t5_title$Ошибка await$m2_l14_t5_title$, $m2_l14_t5_statement$**Коротко:** исправь запуск async-кода.$m2_l14_t5_statement$, $m2_l14_t5_starter$$m2_l14_t5_starter$, $m2_l14_t5_solution$import asyncio

async def main():
    print("hello")

asyncio.run(main())
$m2_l14_t5_solution$, 2, 50, $m2_l14_t5_topic$Месяц 2. Python Hard — Asyncio$m2_l14_t5_topic$, $m2_l14_t5_lang$python$m2_l14_t5_lang$, $m2_l14_t5_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m2_l14_t5_policy$::jsonb),
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m2_l14_ttitle_3$sleep$m2_l14_ttitle_3$,
    $m2_l14_ttitle_4$gather$m2_l14_ttitle_4$,
    $m2_l14_ttitle_5$Ошибка await$m2_l14_ttitle_5$,
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m2_l14_test_3_1_title$sleep$m2_l14_test_3_1_title$, $m2_l14_test_3_1_input$$m2_l14_test_3_1_input$, $m2_l14_test_3_1_expected$start
end
$m2_l14_test_3_1_expected$, FALSE, 1),
  ($m2_l14_test_4_1_title$gather$m2_l14_test_4_1_title$, $m2_l14_test_4_1_input$$m2_l14_test_4_1_input$, $m2_l14_test_4_1_expected$A
B
C
$m2_l14_test_4_1_expected$, FALSE, 1),
  ($m2_l14_test_5_1_title$Ошибка await$m2_l14_test_5_1_title$, $m2_l14_test_5_1_input$$m2_l14_test_5_1_input$, $m2_l14_test_5_1_expected$hello
$m2_l14_test_5_1_expected$, FALSE, 1),
...
Создай async-функцию `main`, которая печатает `start`, делает `await asyncio.sleep(0)`, затем печатает `end`.$m2_l14_b3_content$, $m2_l14_b3_task_title$sleep$m2_l14_b3_task_title$, NULL::jsonb),
...
Создай async-функцию `worker(name)`, которая печатает имя. Запусти три задачи через `asyncio.gather`: `A`, `B`, `C`.$m2_l14_b4_content$, $m2_l14_b4_task_title$gather$m2_l14_b4_task_title$, NULL::jsonb),
  (5, $m2_l14_b5_type$practice$m2_l14_b5_type$, $m2_l14_b5_title$Ошибка await$m2_l14_b5_title$, $m2_l14_b5_content$**Коротко:** исправь запуск async-кода.$m2_l14_b5_content$, $m2_l14_b5_task_title$Ошибка await$m2_l14_b5_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l2_t2_title$Все строки$m3_l2_t2_title$, $m3_l2_t2_statement$**Коротко:** выбери всех пользователей.

### Условие

Напиши SQL-запрос, который возвращает все колонки из таблицы `users`.$m3_l2_t2_statement$, $m3_l2_t2_starter$$m3_l2_t2_starter$, $m3_l2_t2_solution$SELECT *
FROM users;
$m3_l2_t2_solution$, 2, 50, $m3_l2_t2_topic$Месяц 3. SQL и сети — SELECT$m3_l2_t2_topic$, $m3_l2_t2_lang$sql$m3_l2_t2_lang$, $m3_l2_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l2_t2_policy$::jsonb),
  ($m3_l2_t3_title$Колонки$m3_l2_t3_title$, $m3_l2_t3_statement$**Коротко:** выбери имя и город.
...
Анна | Москва
Олег | Казань
```$m3_l2_t3_statement$, $m3_l2_t3_starter$$m3_l2_t3_starter$, $m3_l2_t3_solution$SELECT name, city
FROM users;
$m3_l2_t3_solution$, 2, 50, $m3_l2_t3_topic$Месяц 3. SQL и сети — SELECT$m3_l2_t3_topic$, $m3_l2_t3_lang$sql$m3_l2_t3_lang$, $m3_l2_t3_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l2_t3_policy$::jsonb),
  ($m3_l2_t5_title$Имена$m3_l2_t5_title$, $m3_l2_t5_statement$**Коротко:** выведи названия товаров.

### Условие

Выведи колонку `title` из таблицы `products`. Назови колонку `product_title`.

---$m3_l2_t5_statement$, $m3_l2_t5_starter$$m3_l2_t5_starter$, $m3_l2_t5_solution$SELECT title AS product_title
FROM products;
$m3_l2_t5_solution$, 2, 50, $m3_l2_t5_topic$Месяц 3. SQL и сети — SELECT$m3_l2_t5_topic$, $m3_l2_t5_lang$sql$m3_l2_t5_lang$, $m3_l2_t5_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l2_t5_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l2_ttitle_2$Все строки$m3_l2_ttitle_2$,
    $m3_l2_ttitle_3$Колонки$m3_l2_ttitle_3$,
    $m3_l2_ttitle_5$Имена$m3_l2_ttitle_5$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l2_test_2_1_title$Все строки$m3_l2_test_2_1_title$, $m3_l2_test_2_1_input$$m3_l2_test_2_1_input$, $m3_l2_test_2_1_expected$SELECT *
FROM users;$m3_l2_test_2_1_expected$, TRUE, 1),
  ($m3_l2_test_3_1_title$Колонки$m3_l2_test_3_1_title$, $m3_l2_test_3_1_input$$m3_l2_test_3_1_input$, $m3_l2_test_3_1_expected$SELECT name, city
FROM users;$m3_l2_test_3_1_expected$, TRUE, 1),
  ($m3_l2_test_5_1_title$Имена$m3_l2_test_5_1_title$, $m3_l2_test_5_1_input$$m3_l2_test_5_1_input$, $m3_l2_test_5_1_expected$SELECT title AS product_title
FROM products;$m3_l2_test_5_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Напиши SQL-запрос, который возвращает все колонки из таблицы `users`.$m3_l2_b2_content$, $m3_l2_b2_task_title$Все строки$m3_l2_b2_task_title$, NULL::jsonb),
...
Анна | Москва
Олег | Казань
```$m3_l2_b3_content$, $m3_l2_b3_task_title$Колонки$m3_l2_b3_task_title$, NULL::jsonb),
...
---$m3_l2_b5_content$, $m3_l2_b5_task_title$Имена$m3_l2_b5_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l3_t2_title$Активные$m3_l3_t2_title$, $m3_l3_t2_statement$**Коротко:** выбери активных пользователей.

### Условие

Выведи `name` пользователей, у которых `is_active = TRUE`.$m3_l3_t2_statement$, $m3_l3_t2_starter$$m3_l3_t2_starter$, $m3_l3_t2_solution$SELECT name
FROM users
WHERE is_active = TRUE;
$m3_l3_t2_solution$, 2, 50, $m3_l3_t2_topic$Месяц 3. SQL и сети — WHERE$m3_l3_t2_topic$, $m3_l3_t2_lang$sql$m3_l3_t2_lang$, $m3_l3_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l3_t2_policy$::jsonb),
  ($m3_l3_t4_title$Цена$m3_l3_t4_title$, $m3_l3_t4_statement$**Коротко:** найди товары дороже 1000.

### Условие

Выведи `title` и `price` товаров, у которых `price > 1000` и `in_stock = TRUE`.$m3_l3_t4_statement$, $m3_l3_t4_starter$$m3_l3_t4_starter$, $m3_l3_t4_solution$SELECT title, price
FROM products
WHERE price > 1000 AND in_stock = TRUE;
$m3_l3_t4_solution$, 2, 50, $m3_l3_t4_topic$Месяц 3. SQL и сети — WHERE$m3_l3_t4_topic$, $m3_l3_t4_lang$sql$m3_l3_t4_lang$, $m3_l3_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l3_t4_policy$::jsonb),
  ($m3_l3_t6_title$TOP$m3_l3_t6_title$, $m3_l3_t6_statement$**Коротко:** найди самый дорогой товар.

### Условие

Выведи `title` и `price` самого дорогого товара.$m3_l3_t6_statement$, $m3_l3_t6_starter$$m3_l3_t6_starter$, $m3_l3_t6_solution$SELECT title, price
FROM products
ORDER BY price DESC
LIMIT 1;
$m3_l3_t6_solution$, 2, 50, $m3_l3_t6_topic$Месяц 3. SQL и сети — WHERE$m3_l3_t6_topic$, $m3_l3_t6_lang$sql$m3_l3_t6_lang$, $m3_l3_t6_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l3_t6_policy$::jsonb),
  ($m3_l3_t7_title$Контроль$m3_l3_t7_title$, $m3_l3_t7_statement$**Коротко:** активные взрослые.

### Условие

Выведи `name` и `age` активных пользователей старше или равных 18 лет. Отсортируй по возрасту по возрастанию.

---$m3_l3_t7_statement$, $m3_l3_t7_starter$$m3_l3_t7_starter$, $m3_l3_t7_solution$SELECT name, age
FROM users
WHERE is_active = TRUE AND age >= 18
ORDER BY age ASC;
$m3_l3_t7_solution$, 2, 50, $m3_l3_t7_topic$Месяц 3. SQL и сети — WHERE$m3_l3_t7_topic$, $m3_l3_t7_lang$sql$m3_l3_t7_lang$, $m3_l3_t7_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l3_t7_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l3_ttitle_2$Активные$m3_l3_ttitle_2$,
    $m3_l3_ttitle_4$Цена$m3_l3_ttitle_4$,
    $m3_l3_ttitle_6$TOP$m3_l3_ttitle_6$,
    $m3_l3_ttitle_7$Контроль$m3_l3_ttitle_7$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l3_test_2_1_title$Активные$m3_l3_test_2_1_title$, $m3_l3_test_2_1_input$$m3_l3_test_2_1_input$, $m3_l3_test_2_1_expected$SELECT name
FROM users
WHERE is_active = TRUE;$m3_l3_test_2_1_expected$, TRUE, 1),
  ($m3_l3_test_4_1_title$Цена$m3_l3_test_4_1_title$, $m3_l3_test_4_1_input$$m3_l3_test_4_1_input$, $m3_l3_test_4_1_expected$SELECT title, price
FROM products
WHERE price > 1000 AND in_stock = TRUE;$m3_l3_test_4_1_expected$, TRUE, 1),
  ($m3_l3_test_6_1_title$TOP$m3_l3_test_6_1_title$, $m3_l3_test_6_1_input$$m3_l3_test_6_1_input$, $m3_l3_test_6_1_expected$SELECT title, price
FROM products
ORDER BY price DESC
LIMIT 1;$m3_l3_test_6_1_expected$, TRUE, 1),
  ($m3_l3_test_7_1_title$Контроль$m3_l3_test_7_1_title$, $m3_l3_test_7_1_input$$m3_l3_test_7_1_input$, $m3_l3_test_7_1_expected$SELECT name, age
FROM users
WHERE is_active = TRUE AND age >= 18
ORDER BY age ASC;$m3_l3_test_7_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Выведи `name` пользователей, у которых `is_active = TRUE`.$m3_l3_b2_content$, $m3_l3_b2_task_title$Активные$m3_l3_b2_task_title$, NULL::jsonb),
...
Выведи `title` и `price` товаров, у которых `price > 1000` и `in_stock = TRUE`.$m3_l3_b4_content$, $m3_l3_b4_task_title$Цена$m3_l3_b4_task_title$, NULL::jsonb),
...
Выведи `title` и `price` самого дорогого товара.$m3_l3_b6_content$, $m3_l3_b6_task_title$TOP$m3_l3_b6_task_title$, NULL::jsonb),
...
---$m3_l3_b7_content$, $m3_l3_b7_task_title$Контроль$m3_l3_b7_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l4_t2_title$user orders$m3_l4_t2_title$, $m3_l4_t2_statement$**Коротко:** выведи заказы с именами.

### Условие

Выведи `orders.id`, `users.name`, `orders.status` для всех заказов.$m3_l4_t2_statement$, $m3_l4_t2_starter$$m3_l4_t2_starter$, $m3_l4_t2_solution$SELECT orders.id, users.name, orders.status
FROM orders
JOIN users ON users.id = orders.user_id;
$m3_l4_t2_solution$, 2, 50, $m3_l4_t2_topic$Месяц 3. SQL и сети — JOIN$m3_l4_t2_topic$, $m3_l4_t2_lang$sql$m3_l4_t2_lang$, $m3_l4_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l4_t2_policy$::jsonb),
  ($m3_l4_t4_title$products$m3_l4_t4_title$, $m3_l4_t4_statement$**Коротко:** товар и категория.
...
product_title
category_title
```$m3_l4_t4_statement$, $m3_l4_t4_starter$$m3_l4_t4_starter$, $m3_l4_t4_solution$SELECT products.title AS product_title, categories.title AS category_title
FROM products
JOIN categories ON categories.id = products.category_id;
$m3_l4_t4_solution$, 2, 50, $m3_l4_t4_topic$Месяц 3. SQL и сети — JOIN$m3_l4_t4_topic$, $m3_l4_t4_lang$sql$m3_l4_t4_lang$, $m3_l4_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l4_t4_policy$::jsonb),
  ($m3_l4_t6_title$order total$m3_l4_t6_title$, $m3_l4_t6_statement$**Коротко:** сумма каждого заказа.
...
---$m3_l4_t6_statement$, $m3_l4_t6_starter$$m3_l4_t6_starter$, $m3_l4_t6_solution$SELECT order_id, SUM(quantity * price) AS total
FROM order_items
GROUP BY order_id
ORDER BY order_id;
$m3_l4_t6_solution$, 2, 50, $m3_l4_t6_topic$Месяц 3. SQL и сети — JOIN$m3_l4_t6_topic$, $m3_l4_t6_lang$sql$m3_l4_t6_lang$, $m3_l4_t6_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l4_t6_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l4_ttitle_2$user orders$m3_l4_ttitle_2$,
    $m3_l4_ttitle_4$products$m3_l4_ttitle_4$,
    $m3_l4_ttitle_6$order total$m3_l4_ttitle_6$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l4_test_2_1_title$user orders$m3_l4_test_2_1_title$, $m3_l4_test_2_1_input$$m3_l4_test_2_1_input$, $m3_l4_test_2_1_expected$SELECT orders.id, users.name, orders.status
FROM orders
JOIN users ON users.id = orders.user_id;$m3_l4_test_2_1_expected$, TRUE, 1),
  ($m3_l4_test_4_1_title$products$m3_l4_test_4_1_title$, $m3_l4_test_4_1_input$$m3_l4_test_4_1_input$, $m3_l4_test_4_1_expected$SELECT products.title AS product_title, categories.title AS category_title
FROM products
JOIN categories ON categories.id = products.category_id;$m3_l4_test_4_1_expected$, TRUE, 1),
  ($m3_l4_test_6_1_title$order total$m3_l4_test_6_1_title$, $m3_l4_test_6_1_input$$m3_l4_test_6_1_input$, $m3_l4_test_6_1_expected$SELECT order_id, SUM(quantity * price) AS total
FROM order_items
GROUP BY order_id
ORDER BY order_id;$m3_l4_test_6_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Выведи `orders.id`, `users.name`, `orders.status` для всех заказов.$m3_l4_b2_content$, $m3_l4_b2_task_title$user orders$m3_l4_b2_task_title$, NULL::jsonb),
...
product_title
category_title
```$m3_l4_b4_content$, $m3_l4_b4_task_title$products$m3_l4_b4_task_title$, NULL::jsonb),
...
---$m3_l4_b6_content$, $m3_l4_b6_task_title$order total$m3_l4_b6_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l5_t2_title$count$m3_l5_t2_title$, $m3_l5_t2_statement$**Коротко:** посчитай пользователей.

### Условие

Выведи количество пользователей.$m3_l5_t2_statement$, $m3_l5_t2_starter$$m3_l5_t2_starter$, $m3_l5_t2_solution$SELECT COUNT(*) AS users_count
FROM users;
$m3_l5_t2_solution$, 2, 50, $m3_l5_t2_topic$Месяц 3. SQL и сети — GROUP BY$m3_l5_t2_topic$, $m3_l5_t2_lang$sql$m3_l5_t2_lang$, $m3_l5_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l5_t2_policy$::jsonb),
  ($m3_l5_t4_title$by city$m3_l5_t4_title$, $m3_l5_t4_statement$**Коротко:** пользователи по городам.

### Условие

Выведи город и количество пользователей в нём. Отсортируй по городу.$m3_l5_t4_statement$, $m3_l5_t4_starter$$m3_l5_t4_starter$, $m3_l5_t4_solution$SELECT city, COUNT(*) AS count
FROM users
GROUP BY city
ORDER BY city;
$m3_l5_t4_solution$, 2, 50, $m3_l5_t4_topic$Месяц 3. SQL и сети — GROUP BY$m3_l5_t4_topic$, $m3_l5_t4_lang$sql$m3_l5_t4_lang$, $m3_l5_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l5_t4_policy$::jsonb),
  ($m3_l5_t6_title$revenue$m3_l5_t6_title$, $m3_l5_t6_statement$**Коротко:** выручка по статусу заказа.
...
---$m3_l5_t6_statement$, $m3_l5_t6_starter$$m3_l5_t6_starter$, $m3_l5_t6_solution$SELECT orders.status, SUM(order_items.quantity * order_items.price) AS total
FROM orders
JOIN order_items ON order_items.order_id = orders.id
GROUP BY orders.status
ORDER BY orders.status;
$m3_l5_t6_solution$, 2, 50, $m3_l5_t6_topic$Месяц 3. SQL и сети — GROUP BY$m3_l5_t6_topic$, $m3_l5_t6_lang$sql$m3_l5_t6_lang$, $m3_l5_t6_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l5_t6_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l5_ttitle_2$count$m3_l5_ttitle_2$,
    $m3_l5_ttitle_4$by city$m3_l5_ttitle_4$,
    $m3_l5_ttitle_6$revenue$m3_l5_ttitle_6$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l5_test_2_1_title$count$m3_l5_test_2_1_title$, $m3_l5_test_2_1_input$$m3_l5_test_2_1_input$, $m3_l5_test_2_1_expected$SELECT COUNT(*) AS users_count
FROM users;$m3_l5_test_2_1_expected$, TRUE, 1),
  ($m3_l5_test_4_1_title$by city$m3_l5_test_4_1_title$, $m3_l5_test_4_1_input$$m3_l5_test_4_1_input$, $m3_l5_test_4_1_expected$SELECT city, COUNT(*) AS count
FROM users
GROUP BY city
ORDER BY city;$m3_l5_test_4_1_expected$, TRUE, 1),
  ($m3_l5_test_6_1_title$revenue$m3_l5_test_6_1_title$, $m3_l5_test_6_1_input$$m3_l5_test_6_1_input$, $m3_l5_test_6_1_expected$SELECT orders.status, SUM(order_items.quantity * order_items.price) AS total
FROM orders
JOIN order_items ON order_items.order_id = orders.id
GROUP BY orders.status
ORDER BY orders.status;$m3_l5_test_6_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Выведи количество пользователей.$m3_l5_b2_content$, $m3_l5_b2_task_title$count$m3_l5_b2_task_title$, NULL::jsonb),
...
Выведи город и количество пользователей в нём. Отсортируй по городу.$m3_l5_b4_content$, $m3_l5_b4_task_title$by city$m3_l5_b4_task_title$, NULL::jsonb),
...
---$m3_l5_b6_content$, $m3_l5_b6_task_title$revenue$m3_l5_b6_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l6_t2_title$expensive$m3_l6_t2_title$, $m3_l6_t2_statement$**Коротко:** товары дороже среднего.

### Условие

Выведи `title` и `price` товаров, цена которых выше средней цены всех товаров.$m3_l6_t2_statement$, $m3_l6_t2_starter$$m3_l6_t2_starter$, $m3_l6_t2_solution$SELECT title, price
FROM products
WHERE price > (
    SELECT AVG(price)
    FROM products
);
$m3_l6_t2_solution$, 2, 50, $m3_l6_t2_topic$Месяц 3. SQL и сети — CTE$m3_l6_t2_topic$, $m3_l6_t2_lang$sql$m3_l6_t2_lang$, $m3_l6_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l6_t2_policy$::jsonb),
  ($m3_l6_t4_title$totals$m3_l6_t4_title$, $m3_l6_t4_statement$**Коротко:** заказы дороже 3000.

### Условие

Через CTE посчитай сумму каждого заказа и выведи только заказы с суммой больше `3000`.$m3_l6_t4_statement$, $m3_l6_t4_starter$$m3_l6_t4_starter$, $m3_l6_t4_solution$WITH order_totals AS (
...
WHERE total > 3000
ORDER BY order_id;
$m3_l6_t4_solution$, 2, 50, $m3_l6_t4_topic$Месяц 3. SQL и сети — CTE$m3_l6_t4_topic$, $m3_l6_t4_lang$sql$m3_l6_t4_lang$, $m3_l6_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l6_t4_policy$::jsonb),
  ($m3_l6_t5_title$Контроль$m3_l6_t5_title$, $m3_l6_t5_statement$**Коротко:** пользователи с оплаченной суммой.

### Условие

Через CTE посчитай сумму только оплаченных заказов (`status = 'paid'`) по каждому пользователю. Выведи имя пользователя и сумму.

---$m3_l6_t5_statement$, $m3_l6_t5_starter$$m3_l6_t5_starter$, $m3_l6_t5_solution$WITH paid_orders AS (
    SELECT orders.user_id, SUM(order_items.quantity * order_items.price) AS total
    FROM orders
    JOIN order_items ON order_items.order_id = orders.id
    WHERE orders.status = 'paid'
    GROUP BY orders.user_id
)
SELECT users.name, paid_orders.total
FROM paid_orders
JOIN users ON users.id = paid_orders.user_id
ORDER BY users.name;
$m3_l6_t5_solution$, 2, 50, $m3_l6_t5_topic$Месяц 3. SQL и сети — CTE$m3_l6_t5_topic$, $m3_l6_t5_lang$sql$m3_l6_t5_lang$, $m3_l6_t5_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l6_t5_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l6_ttitle_2$expensive$m3_l6_ttitle_2$,
    $m3_l6_ttitle_4$totals$m3_l6_ttitle_4$,
    $m3_l6_ttitle_5$Контроль$m3_l6_ttitle_5$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l6_test_2_1_title$expensive$m3_l6_test_2_1_title$, $m3_l6_test_2_1_input$$m3_l6_test_2_1_input$, $m3_l6_test_2_1_expected$SELECT title, price
FROM products
WHERE price > (
    SELECT AVG(price)
    FROM products
);$m3_l6_test_2_1_expected$, TRUE, 1),
  ($m3_l6_test_4_1_title$totals$m3_l6_test_4_1_title$, $m3_l6_test_4_1_input$$m3_l6_test_4_1_input$, $m3_l6_test_4_1_expected$WITH order_totals AS (
    SELECT order_id, SUM(quantity * price) AS total
    FROM order_items
    GROUP BY order_id
)
...
FROM order_totals
WHERE total > 3000
ORDER BY order_id;$m3_l6_test_4_1_expected$, TRUE, 1),
  ($m3_l6_test_5_1_title$Контроль$m3_l6_test_5_1_title$, $m3_l6_test_5_1_input$$m3_l6_test_5_1_input$, $m3_l6_test_5_1_expected$WITH paid_orders AS (
    SELECT orders.user_id, SUM(order_items.quantity * order_items.price) AS total
    FROM orders
    JOIN order_items ON order_items.order_id = orders.id
    WHERE orders.status = 'paid'
...
FROM paid_orders
JOIN users ON users.id = paid_orders.user_id
ORDER BY users.name;$m3_l6_test_5_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Выведи `title` и `price` товаров, цена которых выше средней цены всех товаров.$m3_l6_b2_content$, $m3_l6_b2_task_title$expensive$m3_l6_b2_task_title$, NULL::jsonb),
...
Через CTE посчитай сумму каждого заказа и выведи только заказы с суммой больше `3000`.$m3_l6_b4_content$, $m3_l6_b4_task_title$totals$m3_l6_b4_task_title$, NULL::jsonb),
...
---$m3_l6_b5_content$, $m3_l6_b5_task_title$Контроль$m3_l6_b5_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l7_t4_title$Student$m3_l7_t4_title$, $m3_l7_t4_statement$**Коротко:** создай таблицу студентов.
...
- `name` — text, not null;
- `age` — integer, age >= 0;
- `email` — text, unique.$m3_l7_t4_statement$, $m3_l7_t4_starter$$m3_l7_t4_starter$, $m3_l7_t4_solution$CREATE TABLE students (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL,
    age INTEGER NOT NULL CHECK (age >= 0),
    email TEXT UNIQUE
);
$m3_l7_t4_solution$, 2, 50, $m3_l7_t4_topic$Месяц 3. SQL и сети — Schema$m3_l7_t4_topic$, $m3_l7_t4_lang$sql$m3_l7_t4_lang$, $m3_l7_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l7_t4_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l7_ttitle_4$Student$m3_l7_ttitle_4$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l7_test_4_1_title$Student$m3_l7_test_4_1_title$, $m3_l7_test_4_1_input$$m3_l7_test_4_1_input$, $m3_l7_test_4_1_expected$CREATE TABLE students (
    id INTEGER PRIMARY KEY,
    name TEXT NOT NULL,
    age INTEGER NOT NULL CHECK (age >= 0),
    email TEXT UNIQUE
);$m3_l7_test_4_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
- `name` — text, not null;
- `age` — integer, age >= 0;
- `email` — text, unique.$m3_l7_b4_content$, $m3_l7_b4_task_title$Student$m3_l7_b4_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l8_t2_title$Add user$m3_l8_t2_title$, $m3_l8_t2_statement$**Коротко:** добавь пользователя.
...
Затем выведи его имя и город.$m3_l8_t2_statement$, $m3_l8_t2_starter$$m3_l8_t2_starter$, $m3_l8_t2_solution$INSERT INTO users (id, name, city, age, is_active)
VALUES (6, 'Пётр', 'Тула', 22, TRUE);

SELECT name, city
FROM users
WHERE id = 6;
$m3_l8_t2_solution$, 2, 50, $m3_l8_t2_topic$Месяц 3. SQL и сети — DML$m3_l8_t2_topic$, $m3_l8_t2_lang$sql$m3_l8_t2_lang$, $m3_l8_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l8_t2_policy$::jsonb),
  ($m3_l8_t4_title$Activate$m3_l8_t4_title$, $m3_l8_t4_statement$**Коротко:** активируй пользователя.

### Условие

Пользователь с `id = 3` неактивен. Сделай его активным и выведи `name`, `is_active`.$m3_l8_t4_statement$, $m3_l8_t4_starter$$m3_l8_t4_starter$, $m3_l8_t4_solution$UPDATE users
SET is_active = TRUE
WHERE id = 3;
...
FROM users
WHERE id = 3;
$m3_l8_t4_solution$, 2, 50, $m3_l8_t4_topic$Месяц 3. SQL и сети — DML$m3_l8_t4_topic$, $m3_l8_t4_lang$sql$m3_l8_t4_lang$, $m3_l8_t4_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l8_t4_policy$::jsonb),
  ($m3_l8_t6_title$Safe delete$m3_l8_t6_title$, $m3_l8_t6_statement$**Коротко:** безопасное удаление.
...
---$m3_l8_t6_statement$, $m3_l8_t6_starter$$m3_l8_t6_starter$, $m3_l8_t6_solution$SELECT *
FROM orders
WHERE status = 'cancelled';
$m3_l8_t6_solution$, 2, 50, $m3_l8_t6_topic$Месяц 3. SQL и сети — DML$m3_l8_t6_topic$, $m3_l8_t6_lang$sql$m3_l8_t6_lang$, $m3_l8_t6_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l8_t6_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l8_ttitle_2$Add user$m3_l8_ttitle_2$,
    $m3_l8_ttitle_4$Activate$m3_l8_ttitle_4$,
    $m3_l8_ttitle_6$Safe delete$m3_l8_ttitle_6$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l8_test_2_1_title$Add user$m3_l8_test_2_1_title$, $m3_l8_test_2_1_input$$m3_l8_test_2_1_input$, $m3_l8_test_2_1_expected$INSERT INTO users (id, name, city, age, is_active)
VALUES (6, 'Пётр', 'Тула', 22, TRUE);

SELECT name, city
FROM users
WHERE id = 6;$m3_l8_test_2_1_expected$, TRUE, 1),
  ($m3_l8_test_4_1_title$Activate$m3_l8_test_4_1_title$, $m3_l8_test_4_1_input$$m3_l8_test_4_1_input$, $m3_l8_test_4_1_expected$UPDATE users
SET is_active = TRUE
WHERE id = 3;

SELECT name, is_active
FROM users
WHERE id = 3;$m3_l8_test_4_1_expected$, TRUE, 1),
  ($m3_l8_test_6_1_title$Safe delete$m3_l8_test_6_1_title$, $m3_l8_test_6_1_input$$m3_l8_test_6_1_input$, $m3_l8_test_6_1_expected$SELECT *
FROM orders
WHERE status = 'cancelled';$m3_l8_test_6_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Затем выведи его имя и город.$m3_l8_b2_content$, $m3_l8_b2_task_title$Add user$m3_l8_b2_task_title$, NULL::jsonb),
...
Пользователь с `id = 3` неактивен. Сделай его активным и выведи `name`, `is_active`.$m3_l8_b4_content$, $m3_l8_b4_task_title$Activate$m3_l8_b4_task_title$, NULL::jsonb),
...
---$m3_l8_b6_content$, $m3_l8_b6_task_title$Safe delete$m3_l8_b6_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l9_t2_title$BEGIN/COMMIT$m3_l9_t2_title$, $m3_l9_t2_statement$**Коротко:** оформи транзакцию.

### Условие

Напиши SQL-транзакцию, которая меняет статус заказа `id = 2` на `paid`.$m3_l9_t2_statement$, $m3_l9_t2_starter$$m3_l9_t2_starter$, $m3_l9_t2_solution$BEGIN;
...
COMMIT;
$m3_l9_t2_solution$, 2, 50, $m3_l9_t2_topic$Месяц 3. SQL и сети — ACID$m3_l9_t2_topic$, $m3_l9_t2_lang$sql$m3_l9_t2_lang$, $m3_l9_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l9_t2_policy$::jsonb),
  ($m3_l9_t5_title$Transfer$m3_l9_t5_title$, $m3_l9_t5_statement$**Коротко:** опиши транзакцию перевода.
...
---$m3_l9_t5_statement$, $m3_l9_t5_starter$$m3_l9_t5_starter$, $m3_l9_t5_solution$BEGIN;
...
COMMIT;
$m3_l9_t5_solution$, 2, 50, $m3_l9_t5_topic$Месяц 3. SQL и сети — ACID$m3_l9_t5_topic$, $m3_l9_t5_lang$sql$m3_l9_t5_lang$, $m3_l9_t5_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l9_t5_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l9_ttitle_2$BEGIN/COMMIT$m3_l9_ttitle_2$,
    $m3_l9_ttitle_5$Transfer$m3_l9_ttitle_5$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l9_test_2_1_title$BEGIN/COMMIT$m3_l9_test_2_1_title$, $m3_l9_test_2_1_input$$m3_l9_test_2_1_input$, $m3_l9_test_2_1_expected$BEGIN;

UPDATE orders
SET status = 'paid'
WHERE id = 2;

COMMIT;$m3_l9_test_2_1_expected$, TRUE, 1),
  ($m3_l9_test_5_1_title$Transfer$m3_l9_test_5_1_title$, $m3_l9_test_5_1_input$$m3_l9_test_5_1_input$, $m3_l9_test_5_1_expected$BEGIN;
...
COMMIT;$m3_l9_test_5_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Напиши SQL-транзакцию, которая меняет статус заказа `id = 2` на `paid`.$m3_l9_b2_content$, $m3_l9_b2_task_title$BEGIN/COMMIT$m3_l9_b2_task_title$, NULL::jsonb),
...
---$m3_l9_b5_content$, $m3_l9_b5_task_title$Transfer$m3_l9_b5_task_title$, NULL::jsonb)
), resolved AS (
  SELECT bs.position, bs.block_type, bs.title, bs.content_md, tl.task_id, bs.quiz_payload
  FROM block_seed bs
  LEFT JOIN task_lookup tl ON tl.title = bs.task_title
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l11_t2_title$CREATE INDEX$m3_l11_t2_title$, $m3_l11_t2_statement$**Коротко:** создай индекс.

### Условие

Создай индекс `idx_users_city` на колонку `city` таблицы `users`.$m3_l11_t2_statement$, $m3_l11_t2_starter$$m3_l11_t2_starter$, $m3_l11_t2_solution$CREATE INDEX idx_users_city
ON users(city);
$m3_l11_t2_solution$, 2, 50, $m3_l11_t2_topic$Месяц 3. SQL и сети — Indexes$m3_l11_t2_topic$, $m3_l11_t2_lang$sql$m3_l11_t2_lang$, $m3_l11_t2_policy${"language":"sql","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l11_t2_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l11_ttitle_2$CREATE INDEX$m3_l11_ttitle_2$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l11_test_2_1_title$CREATE INDEX$m3_l11_test_2_1_title$, $m3_l11_test_2_1_input$$m3_l11_test_2_1_input$, $m3_l11_test_2_1_expected$CREATE INDEX idx_users_city
ON users(city);$m3_l11_test_2_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
Создай индекс `idx_users_city` на колонку `city` таблицы `users`.$m3_l11_b2_content$, $m3_l11_b2_task_title$CREATE INDEX$m3_l11_b2_task_title$, NULL::jsonb),
...
$m3_l12_t2_solution$, 2, 50, $m3_l12_t2_topic$Месяц 3. SQL и сети — sqlite3$m3_l12_t2_topic$, $m3_l12_t2_lang$python$m3_l12_t2_lang$, $m3_l12_t2_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l12_t2_policy$::jsonb),
  ($m3_l12_t3_title$insert$m3_l12_t3_title$, $m3_l12_t3_statement$**Коротко:** добавь пользователя.

### Условие

Создай таблицу `users(id, name)`, добавь пользователя `Анна`, сохрани изменения и выведи `OK`.$m3_l12_t3_statement$, $m3_l12_t3_starter$$m3_l12_t3_starter$, $m3_l12_t3_solution$import sqlite3
...
print("OK")
$m3_l12_t3_solution$, 2, 50, $m3_l12_t3_topic$Месяц 3. SQL и сети — sqlite3$m3_l12_t3_topic$, $m3_l12_t3_lang$python$m3_l12_t3_lang$, $m3_l12_t3_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l12_t3_policy$::jsonb),
...
WITH lesson_ref AS (
  ...
  WHERE t.title IN (
    ...
    $m3_l12_ttitle_3$insert$m3_l12_ttitle_3$,
...
WITH lesson_ref AS (
  ...
  ($m3_l12_test_3_1_title$insert$m3_l12_test_3_1_title$, $m3_l12_test_3_1_input$$m3_l12_test_3_1_input$, $m3_l12_test_3_1_expected$OK
$m3_l12_test_3_1_expected$, FALSE, 1),
...
Создай таблицу `users(id, name)`, добавь пользователя `Анна`, сохрани изменения и выведи `OK`.$m3_l12_b3_content$, $m3_l12_b3_task_title$insert$m3_l12_b3_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l13_t3_title$Model$m3_l13_t3_title$, $m3_l13_t3_statement$**Коротко:** создай ORM-модель.
...
- `id`;
- `name`.$m3_l13_t3_statement$, $m3_l13_t3_starter$$m3_l13_t3_starter$, $m3_l13_t3_solution$from sqlalchemy import create_engine, Integer, String
...
print("OK")
$m3_l13_t3_solution$, 2, 50, $m3_l13_t3_topic$Месяц 3. SQL и сети — SQLAlchemy$m3_l13_t3_topic$, $m3_l13_t3_lang$python$m3_l13_t3_lang$, $m3_l13_t3_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m3_l13_t3_policy$::jsonb),
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m3_l13_ttitle_3$Model$m3_l13_ttitle_3$,
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m3_l13_test_3_1_title$Model$m3_l13_test_3_1_title$, $m3_l13_test_3_1_input$$m3_l13_test_3_1_input$, $m3_l13_test_3_1_expected$OK
$m3_l13_test_3_1_expected$, FALSE, 1),
...
- `id`;
- `name`.$m3_l13_b3_content$, $m3_l13_b3_task_title$Model$m3_l13_b3_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m4_l2_t3_title$GET /$m4_l2_t3_title$, $m4_l2_t3_statement$**Коротко:** добавь первый endpoint.
...
```json
{"message": "Hello API"}
```$m4_l2_t3_statement$, $m4_l2_t3_starter$$m4_l2_t3_starter$, $m4_l2_t3_solution$from fastapi import FastAPI
...
$m4_l2_t3_solution$, 2, 50, $m4_l2_t3_topic$Месяц 4. FastAPI и DevOps — FastAPI start$m4_l2_t3_topic$, $m4_l2_t3_lang$python$m4_l2_t3_lang$, $m4_l2_t3_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m4_l2_t3_policy$::jsonb),
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m4_l2_ttitle_3$GET /$m4_l2_ttitle_3$,
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m4_l2_test_3_1_title$GET /$m4_l2_test_3_1_title$, $m4_l2_test_3_1_input$$m4_l2_test_3_1_input$, $m4_l2_test_3_1_expected$__SOURCE_EQ__
...
@app.get("/")
def root():
    return {"message": "Hello API"}$m4_l2_test_3_1_expected$, TRUE, 1),
...
```json
{"message": "Hello API"}
```$m4_l2_b3_content$, $m4_l2_b3_task_title$GET /$m4_l2_b3_task_title$, NULL::jsonb),
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m4_l11_t4_title$API test$m4_l11_t4_title$, $m4_l11_t4_statement$**Коротко:** протестируй healthcheck.
...
- status code 200;
- JSON `{"status": "ok"}`.$m4_l11_t4_statement$, $m4_l11_t4_starter$$m4_l11_t4_starter$, $m4_l11_t4_solution$from fastapi.testclient import TestClient
...
$m4_l11_t4_solution$, 2, 50, $m4_l11_t4_topic$Месяц 4. FastAPI и DevOps — PyTest$m4_l11_t4_topic$, $m4_l11_t4_lang$python$m4_l11_t4_lang$, $m4_l11_t4_policy${"language":"python","emptySourceMessage":"Код пока пуст. Добавьте решение и запустите проверку снова.","ignoreCommentOnlyLines":true,"hints":[],"ai_hint_config":{"mode":"socratic","no_full_solution":true}}$m4_l11_t4_policy$::jsonb)
)
INSERT INTO tasks(lesson_id, title, statement_md, starter_code, solution_code, difficulty, xp_reward, topic, language, source_policy, is_published)
SELECT lr.lesson_id, ts.title, ts.statement_md, ts.starter_code, ts.solution_code, ts.difficulty, ts.xp_reward, ts.topic, ts.language, ts.source_policy, TRUE
FROM lesson_ref lr
...
WITH lesson_ref AS (
  ...
  JOIN lesson_ref lr ON lr.lesson_id = t.lesson_id
  WHERE t.title IN (
    $m4_l11_ttitle_4$API test$m4_l11_ttitle_4$
  )
)
DELETE FROM task_test_cases tc
USING target_tasks tt
...
WITH lesson_ref AS (
  ...
  VALUES
  ($m4_l11_test_4_1_title$API test$m4_l11_test_4_1_title$, $m4_l11_test_4_1_input$$m4_l11_test_4_1_input$, $m4_l11_test_4_1_expected$__SOURCE_EQ__
...
def test_health():
    response = client.get("/health")
    assert response.status_code == 200
    assert response.json() == {"status": "ok"}$m4_l11_test_4_1_expected$, TRUE, 1)
)
INSERT INTO task_test_cases(task_id, input_data, expected_output, is_hidden, position)
SELECT tl.task_id, ts.input_data, ts.expected_output, ts.is_hidden, ts.position
FROM test_seed ts
...
- status code 200;
- JSON `{"status": "ok"}`.$m4_l11_b4_content$, $m4_l11_b4_task_title$API test$m4_l11_b4_task_title$, NULL::jsonb),
