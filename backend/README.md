...
## Актуализация от 2026-05-05: compatibility fields для mixed версий IDE plugin

1. В `GET /api/v1/courses/:courseID/tasks-catalog` и `GET /api/v1/plugin/bootstrap` добавлены дублирующие legacy-поля для совместимости:
- `id` (дубликат `taskId`);
- `order` (дубликат `position`);
- `taskStatus` (дубликат `status`).

2. Причина:
- в production может одновременно работать несколько версий IDE plugin;
