...
function buildQuizPayload(step, modulePosition, lessonPosition, stepPosition) {
  const questions = ensureArray(step.questions).map((question, qIdx) => {
    ...
    if (options.length < 2) {
      throw new Error(
        `Quiz options parse failed at m${modulePosition} l${lessonPosition} s${stepPosition} q${qIdx + 1}`
      );
    }
    const fallbackCorrect = options[0].id;
    const correctId = normalizeOptionId(question.correct_answer_id, 0);
    const normalizedCorrect = options.some((opt) => opt.id === correctId) ? correctId : fallbackCorrect;
  ...
  return {
    type: "single_choice",
    minScorePercent: clampInt(step.checker?.min_score_percent, 0, 100, 67),
    questions,
  };
}

function buildSourcePolicy(step, checkerType, checker, language) {
  const hints = ensureArray(step.hints).map((x) => normalizeSpaces(x)).filter(Boolean);
  const checkerTests = {
    type: checkerType,
    timeout_sec: toInt(checker.timeout_sec, 4),
  ...
  for (const test of ensureArray(checker.public_tests)) {
    checkerTests.tests.push({
      input: String(test.input ?? ""),
      expected_stdout: String(test.expected_stdout ?? ""),
      visibility: "public",
    });
  }
  for (const test of ensureArray(checker.hidden_tests)) {
    checkerTests.tests.push({
      input: String(test.input ?? ""),
      expected_stdout: String(test.expected_stdout ?? ""),
      visibility: "hidden",
  ...
  return {
    language,
    import_source: "materials/java_metanit_v1/course_import.json",
    checker_type: checkerType,
    checker: checkerTests,
    hints,
    ...
    admin_notes: normalizeSpaces(step.admin_notes),
  };
}

function buildTaskFromPractice(step, moduleTitle, lessonTitle) {
  const checker = step.checker || {};
  const checkerType = normalizeCheckerType(checker.type);
  if (checkerType !== "java_stdout") {
    throw new Error(`Unsupported checker for Java course practice step: ${checkerType || "<empty>"}`);
  }

  const task = {
    title: normalizeSpaces(step.title) || "Практика",
    statementMd: sanitizeMultilineText(step.body_markdown || "").trim(),
    starterCode: ensureTrailingNewline(step.editor_initial_code || ""),
    solutionCode: ensureTrailingNewline(step.solution_code || ""),
    difficulty: clampInt(step.difficulty, 1, 10, 2),
    xpReward: Math.max(10, toInt(step.xp, 70) || 70),
    topic: `${normalizeSpaces(moduleTitle)} — ${normalizeSpaces(lessonTitle)}`,
    language: "java",
    sourcePolicy: buildSourcePolicy(step, checkerType, checker, "java"),
    tests: [],
  };
  ...
  for (const test of ensureArray(checker.public_tests)) {
    task.tests.push({
      ...
      expectedOutput: String(test.expected_stdout ?? ""),
      isHidden: false,
      position: pos++,
    });
  }
...
function prepareCourse(rawRoot) {
  ...
  return {
    slug: COURSE_SLUG,
    title: normalizeSpaces(rawCourse.title) || "Java с нуля — Metanit Track",
    description:
      normalizeSpaces(rawCourse.description) ||
      "Полный Java-трек на основе Metanit: подробная теория, квизы и ручные практические задачи с автопроверкой.",
    modules,
  };
}

function buildTaskSeedValues(modulePosition, lessonPosition, lesson) {
...
function buildBlockSeedValues(modulePosition, lessonPosition, lesson) {
  ...
  for (let i = 0; i < lesson.blocks.length; i += 1) {
    ...
    const taskTitleValue = block.task
      ? dollarQuote(block.task.title, `m${modulePosition}_l${lessonPosition}_b${block.position}_task`)
      : "NULL";
...
function buildMigrationSql(course) {
  ...
  for (const moduleItem of modules) {
    ...
    for (const lesson of lessons) {
      ...
      const taskSeedSql = buildTaskSeedValues(moduleItem.modulePosition, lesson.lessonPosition, lesson);
      if (taskSeedSql !== "") {
        sql += "WITH lesson_ref AS (\n";
        sql += "  SELECT l.id AS lesson_id\n";
        sql += "  FROM lessons l\n";
  ...
  for (const moduleItem of course.modules) {
    ...
    for (const lesson of moduleItem.lessons) {
      ...
      for (const block of lesson.blocks) {
        ...
        if (checkerType) stats.checkerTypes[checkerType] = (stats.checkerTypes[checkerType] || 0) + 1;
        const lang = normalizeSpaces(block.task.language).toLowerCase() || "java";
...
function main() {
  ...
  const migrationSQL = buildMigrationSql(course).replace(/\n+$/, "\n");
