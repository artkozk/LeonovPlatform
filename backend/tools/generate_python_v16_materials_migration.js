...
const INPUT_PATH = path.resolve(__dirname, '../../материалы/course_import.json');
const OUT_PATH = path.resolve(__dirname, '../migrations/025_reseed_python_zero_v16_materials.sql');
...
function buildSourcePolicy(step, checkerType, checkerConverted, language) {
  ...
  return {
    language,
    import_source: 'materials/course_import.json',
...
function buildMigrationSql(course) {
  let sql = '';
  sql += '-- 025_reseed_python_zero_v16_materials.sql\n';
  sql += '-- Generated from materials/course_import.json\n';
  sql += '-- Purpose: fully reseed python-zero from materials package with runtime-compatible checker configs.\n';
