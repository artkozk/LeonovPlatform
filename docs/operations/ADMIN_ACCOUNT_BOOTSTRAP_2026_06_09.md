...
## Дополнение 2026-06-16: поддержка логина по username для админа

### Что всплыло после перекатки

1. На проде часть команд и пользователей продолжали вызывать API как `{"email":"admin",...}` после предыдущего фикса, тогда как backend ждал только email.
2. Были замечены смешанные отчеты: логин с `admin@platform.local` проходил, с `admin` — не всегда, в зависимости от версии backend и статуса кэша.

### Что исправлено

1. В `backend/internal/app/handlers_auth.go` в `Login` добавлен режим выбора по входным данным:
   - если строка содержит `@`, выполняется нормализация email и поиск по `u.email`;
   - иначе поиск идёт по `u.username`.
2. Сервисный доступ на production обновлён вместе с последним деплоем (`current` переключён на свежий релиз, выполнен сборка и health-check).

### Проверка

- `POST /api/v1/auth/login` с `{"email":"admin", "password":"123123123"}` → `200`.
- `POST /api/v1/auth/login` с `{"email":"admin@platform.local", "password":"123123123"}` → `200`.
- Ручная проверка токена через `GET /api/v1/auth/me` и административный профиль проходит.
