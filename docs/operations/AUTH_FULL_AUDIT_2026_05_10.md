...
1. Выполнены попытки SSH-подключения к серверу:
- `ssh root@85.198.82.221`;
- `sshpass ... ssh root@85.198.82.221 ...`;
- `plink -ssh -pw ... root@85.198.82.221 ...`;
- проверка через Python/Paramiko.
2. Фактический результат:
- сервер доступен по сети (`ping` успешен);
- SSH возвращает `Access denied` / `Authentication failed` для предоставленных данных `root@85.198.82.221` + `Publish_Tw36gi`.

### 7.2 Текущий статус post-deploy re-check

1. Автоматический деплой и post-deploy прогон auth-матрицы заблокированы до получения валидного SSH-доступа.
2. Код и документация уже закоммичены и запушены в `origin/main`:
- commit: `268ade1`;
- push: `main -> origin/main`.
3. После предоставления рабочего SSH-доступа необходимо выполнить:
- `git pull` на сервере;
- запуск `deploy/server/deploy.sh`;
- повторный прогон auth-матрицы из раздела 2 на production;
- append-only фиксацию фактических post-deploy результатов в этом же документе.
