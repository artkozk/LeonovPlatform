...
## 13. Актуализация от 2026-05-16: blueprint внедрения support-чата (design до кода)

1. Для внедрения встроенной поддержки подготовлен отдельный архитектурный blueprint:
- `docs/architecture/SUPPORT_CHAT_IMPLEMENTATION_BLUEPRINT_2026_05_16.md`.
2. В документе зафиксированы:
- продуктовая state-machine обращения;
- модель операторов и аудит передачи ответственности;
- realtime-контур и эксплуатационные ограничения;
- проектный контракт БД/API;
- security и anti-regression требования.
3. Это добавлено до начала реализации, чтобы внедрение выполнялось как системная доработка платформы, а не как локальный UI/endpoint костыль.
