...
func (s *Server) createChatMessage(ctx context.Context, threadID string, user User, messageType, body, replyToID, linkedRecordID, attachmentID string) (ChatMessage, error) {
	...
	defer tx.Rollback()
	if _, err = tx.ExecContext(ctx, `INSERT INTO chat_messages(id, thread_id, author_id, reply_to_id, linked_record_id, attachment_id, message_type, body, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`, id, threadID, user.ID, nullable(replyToID), nullable(linkedRecordID), nullable(attachmentID), messageType, body, now); err != nil {
		return ChatMessage{}, err
	}
...
func (s *Server) handleCreateChatMessage(w http.ResponseWriter, r *http.Request) {
	...
	message, err := s.createChatMessage(r.Context(), threadID, currentUser(r), "text", input.Body, input.ReplyToID, input.LinkedRecordID, "")
	if err != nil {
