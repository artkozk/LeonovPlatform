...
func (s *Server) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.store.db.QueryContext(r.Context(), `
		SELECT id, entity_kind, record_id, question_id, type, title, context, status, updated_at
		FROM (
			SELECT 'record:' || r.id AS id, 'record' AS entity_kind, r.id AS record_id, '' AS question_id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&result.ID, &result.EntityKind, &result.RecordID, &result.QuestionID, &result.Type, &result.Title, &result.Context, &result.Status, &result.UpdatedAt); err != nil {
