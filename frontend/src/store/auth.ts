...
export const useAuthStore = create<AuthStore>((set) => ({
  ...
  bootstrap: async () => {
    ...
    try {
      ...
      // Транзиентный сбой /me (сеть, 5xx) — не сносим сессию, но и
      // не зависаем в bootstrap-стейте: `bootstrapped: true` снимает
      // блокировку UI, а error даёт пользователю обратную связь.
      set({
        loading: false,
        bootstrapped: true,
