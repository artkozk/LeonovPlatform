...
### Актуализация от 2026-05-29: подписки-only, промокоды, аудит запуска

1. Материалы переведены в строгий subscription-only режим:
- технический `free` план сохранён только как fallback для аккаунта без оплаты;
- `free` больше не публикуется в списке тарифов и не даёт доступ к курсам;
- доступ к контенту теперь проверяется на backend для всех ключевых сценариев:
  - список курсов;
  - курс/урок/задача;
  - запуск и отправка решения;
  - квизы и прогресс;
  - AI-подсказки;
  - plugin endpoints (`template/style-check/reference/progress/sync`).
2. Тарифы уточнены продуктово:
- `Start` (`pro`) — доступ к 1 курсу;
- `Premium` — доступ ко всем курсам + AI-подсказки;
- добавлено поле `plans.max_courses` для явной серверной модели доступа.
3. Внедрена система одноразовых промокодов:
- таблица `promo_codes`;
- endpoint: `POST /api/v1/subscription/promocode/apply`;
- базовый режим для промокодов — одноразовый.
4. Очищена мусорная статистика и нормализованы рейтинги:
- удаление неконсистентных `xp_events` (чужой `submission_id`);
- полный пересчёт `users.xp` и `users.level` по фактическим `xp_events`;
- пересчёт streak по дневной семантике;
- продуктовый буст аккаунта `artkozk` (макс. XP/уровень для лидерборда).
5. Подробный технический отчёт и причины решений:
- `docs/operations/SUBSCRIPTION_ONLY_ACCESS_PROMOCODES_AUDIT_2026_05_29.md`.

### Актуализация от 2026-06-17: безлимитный режим для 20 промокодов

1. Добавлен режим `is_reusable` в `promo_codes` для целевого набора `20` промокодов.
2. Для этих кодов включено многоразовое применение по бизнес-требованию.
3. Режим реализован так, чтобы legacy одноразовые промокоды продолжали возвращать 409 после первого использования.
